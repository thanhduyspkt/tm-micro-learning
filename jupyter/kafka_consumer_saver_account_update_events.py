...
'''
An AccountUpdateEvent is generated when an account update event is published.
    1. Account update creation (AccountUpdateCreatedEvent)
    2. Updates of account update (AccountUpdateUpdatedEvent)
'''
...
def main():
    ...
    try:
        ...
        topic = ACCOUNT_UPDATE_EVENTS_TOPIC
...
def insert_into_mongo_db(db,record_consumed):
    try:
        db.account_update_events.insert_one(record_consumed)
    except:
